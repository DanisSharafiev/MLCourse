...
IndexHNSW
Создается граф, с помощью efSearch выбирается сколько графов мы посетим. При создании эмбеддинга
...
- **RoBERTa, DistilBERT, ALBERT** - улучшения BERT
...
- **Tools** - интеграция с внешними API и сервисами
...
- **Transformer** - базовая архитектура (attention mechanism, encoder-decoder)
- **GPT** (decoder-only) vs **BERT** (encoder-only) vs **T5** (encoder-decoder)
- **LLaMA, Mistral, Claude** - современные модели
- **Mixture of Experts (MoE)** - архитектура для масштабирования